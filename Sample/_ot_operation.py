...
def semua_data_overtime():
    ...
    no = 1
    for th in _ot_calculation.ambil_data():
        for u in _ot_calculation.ambil_data()[th]:
...
def data_overtime():
    if len(_ot_calculation.ambil_data()) != 0:
        ...
        if ouwe == '1':
            ...
            for i in _ot_calculation.ambil_data():
                print(i)
                for u in _ot_calculation.ambil_data()[i]:
                    print('\t| -',u)
            ...
            if tahun in _ot_calculation.ambil_data().keys():
                ambil_bulanan = _ot_calculation.ambil_data()[tahun]
                if bulan in ambil_bulanan.keys():
                    ...
                    for i in ambil_harian:
                        print(
                            f"{no}\t\t{i["Tanggal"]}\t\t\t{i["Jam lembur"]}\t\t\t\t\t\t{i["Perkalian jam lembur"]}\t\t\t\t\t\t{_ot_calculation.format_rupiah(i["Nominal uang lembur"])}")
                        no += 1
                    print("=" * 100)
                else:
...
def input_overtime():
    nilai_gapok = _ot_calculation.ambil_gapok()
    data_tahunan = _ot_calculation.ambil_data()
    ...
    try:
        ...
        _ot_calculation.rumus(tahun, bulan, tanggal, jam_lembur)
        ...
        if len(data_tahunan) > 0:
            if tahun in data_tahunan.keys():
                ...
                if bulan in data_bulanan.keys():
                    ...
                    if len(data_harian) > 0:
                        ...
                        if b:
                            data_harian.append({'Tanggal': f"{tanggal}/{bulan}/{tahun}", 'Jam lembur': jam_lembur,
                                                'Perkalian jam lembur': _ot_calculation.hasil_perkalian_jam,
                                                'Nominal uang lembur': _ot_calculation.hasil_uang_lemburan})
                            _ot_calculation.input_data(data_tahunan)
                        else:
                                ...
                                while i < len(data_harian):
                                    if f"{tanggal}/{bulan}/{tahun}" in data_harian[i].values():
                                        data_harian[i] = {'Tanggal': f"{tanggal}/{bulan}/{tahun}", 'Jam lembur': jam_lembur,
                                                          ...
                                                          'Nominal uang lembur': _ot_calculation.hasil_uang_lemburan}
                                    i += 1
                                _ot_calculation.edit_data(data_tahunan)
                            else:
                    ...
                    data_bulanan[bulan] = [{'Tanggal': f"{tanggal}/{bulan}/{tahun}", 'Jam lembur': jam_lembur, 'Perkalian jam lembur': _ot_calculation.hasil_perkalian_jam, 'Nominal uang lembur': _ot_calculation.hasil_uang_lemburan}]
                    _ot_calculation.input_data(data_tahunan)
            else:
                data_tahunan[tahun] = {bulan:[{'Tanggal': f"{tanggal}/{bulan}/{tahun}", 'Jam lembur': jam_lembur, 'Perkalian jam lembur': _ot_calculation.hasil_perkalian_jam, 'Nominal uang lembur': _ot_calculation.hasil_uang_lemburan}]}
                _ot_calculation.input_data(data_tahunan)
        else:
            data_harian = [{'Tanggal': f"{tanggal}/{bulan}/{tahun}", 'Jam lembur': jam_lembur, 'Perkalian jam lembur': _ot_calculation.hasil_perkalian_jam, 'Nominal uang lembur': _ot_calculation.hasil_uang_lemburan}]
...
def edit_overtime():
    if len(_ot_calculation.ambil_data()) != 0:
        ...
        for i in _ot_calculation.ambil_data():
            ...
            for u in _ot_calculation.ambil_data()[i]:
                print('\t| -', u)
                for o in _ot_calculation.ambil_data()[i][u]:
                    print('\t\t  | -', o['Tanggal'])
                    hoho += 1
        ...
        tgl = input()
        jos = _ot_calculation.ambil_data()
        dd = tgl[:2]
        mm = tgl[3:5]
        yyyy = tgl[6:10]
        try:
            ...
            _ot_calculation.rumus(yyyy, mm, dd, jam_lembur)
...
def hapus_overtime():
    if len(_ot_calculation.ambil_data()) != 0:
        ...
        if i == '1':
            ...
            if joe in yos:
                yos.pop(joe)
                _ot_calculation.hapus_data(yos)
